...
public class CodeGenSubgraphWorkflow {
    ...
    private StateGraph<MessagesState<String>> createContentImageSubgraph() {
        try {
            // 子图只有一个内容图片收集节点，父图会把它当成一个整体节点来调度。
    ...
    private StateGraph<MessagesState<String>> createIllustrationSubgraph() {
        try {
            // 插画收集子图独立封装，方便以后给插画流程增加多个步骤而不影响父图结构。
            return new MessagesStateGraph<String>()
    ...
    private StateGraph<MessagesState<String>> createDiagramSubgraph() {
        try {
            // 架构图/流程图生成也是独立子图，后续可以扩展为“分析需求 -> 生成 Mermaid -> 渲染图片”。
            return new MessagesStateGraph<String>()
    ...
    private StateGraph<MessagesState<String>> createLogoSubgraph() {
        try {
            // Logo 生成子图目前只有一个节点，保持子图形式是为了和其他素材流程统一。
            return new MessagesStateGraph<String>()
    ...
    public CompiledGraph<MessagesState<String>> createWorkflow() {
        try {
            // 获取各个未编译的子图（跟父图完全共享状态），子图内写入的图片资源会回到同一个 WorkflowContext。
            ...
            return new MessagesStateGraph<String>()
                    ...
                    // 添加子图作为节点：父图不关心子图内部细节，只关心输入状态和输出状态。
                    ...
                    // 添加图片聚合节点：四个子图完成后，把不同来源的图片资源统一整理。
                    .addNode("image_aggregator", ImageAggregatorNode.create())

                    // 添加边 - 串行部分：先由 image_plan 决定需要哪些素材。
                    .addEdge(START, "image_plan")
                    
                    // 并发子图分支：从计划节点分发到各个子图，各子图共享同一个业务上下文。
                    ...
                    // 汇聚：所有子图都汇聚到聚合器，避免后续 prompt_enhancer 需要理解多份分散结果。
                    ...
                    // 继续串行流程：聚合图片后增强 prompt，再路由到具体代码生成模式。
                    .addEdge("image_aggregator", "prompt_enhancer")
                    .addEdge("prompt_enhancer", "router")
                    .addEdge("router", "code_generator")
                    .addEdge("code_generator", "code_quality_check")

                    // 质检条件边：质检失败时回到 code_generator，质检通过后按项目类型决定是否构建。
            ...
            // 子图或父图结构非法时，编译阶段会抛 GraphStateException。
    ...
    private String routeAfterQualityCheck(MessagesState<String> state) {
        // 条件边会把当前图状态传进来，这里取出业务上下文判断下一条边。
        WorkflowContext context = WorkflowContext.getContext(state);
        QualityResult qualityResult = context.getQualityResult();

        // 没有质检结果也视为失败，避免未校验代码直接进入构建/结束。
    ...
    public WorkflowContext executeWorkflow(String originalPrompt) {
        ...
        // 初始化上下文，后续所有节点都围绕这份 WorkflowContext 读写状态。
        ...
        int stepCounter = 1;
        // 逐步执行工作流，stream 每产生一次输出就表示一个节点或子图阶段完成。
