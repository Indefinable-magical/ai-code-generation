...
import com.yu.aicodeGeneration.mapper.ChatHistoryMapper;
 ...
 * 对话历史有两种用途：
 * 1. 给前端展示历史聊天记录；
 * 2. 加载到 LangChain4j 的 ChatMemory，让 AI 具备多轮上下文。
 */
@Service
@Slf4j
public class ChatHistoryServiceImpl extends ServiceImpl<ChatHistoryMapper, ChatHistory> implements ChatHistoryService {
    ...
    public Page<ChatHistory> listAppChatHistoryByPage(Long appId, int pageSize,
                                                      ...
                                                      User loginUser) {
        // 分页查询对话历史，使用 lastCreateTime 做游标，适合聊天列表“加载更多”。
        ThrowUtils.throwIf(appId == null || appId <= 0, ErrorCode.PARAMS_ERROR, "应用ID不能为空");
        ...
        // 构建查询条件，把 appId 和游标时间传给 getQueryWrapper。
        ChatHistoryQueryRequest queryRequest = new ChatHistoryQueryRequest();
        queryRequest.setAppId(appId);
        queryRequest.setLastCreateTime(lastCreateTime);
        QueryWrapper queryWrapper = this.getQueryWrapper(queryRequest);
        // 查询第一页 pageSize 条；游标条件已经限制了从哪个时间点继续往前翻。
    ...
    public int loadChatHistoryToMemory(Long appId, MessageWindowChatMemory chatMemory, int maxCount) {
        try {
            // 查询最近 maxCount 条对话，先按 createTime 倒序拿最新数据。
            QueryWrapper queryWrapper = QueryWrapper.create()
            ...
            // 模型上下文必须按真实对话顺序加入，否则会理解错先后关系。
            historyList = historyList.reversed();
            // 按照时间顺序将消息添加到记忆中。
            int loadedCount = 0;
            // 先清理历史缓存，防止重复加载导致同一批消息出现两次。
            chatMemory.clear();
            for (ChatHistory history : historyList) {
                // 用户消息要转成 LangChain4j 的 UserMessage。
    ...
    public QueryWrapper getQueryWrapper(ChatHistoryQueryRequest chatHistoryQueryRequest) {
        // 如果查询对象为空，返回无条件 QueryWrapper，调用方可自行追加分页。
        QueryWrapper queryWrapper = QueryWrapper.create();
        ...
        // 拼接查询条件：空值会被 MyBatis-Flex 忽略。
        queryWrapper.eq("id", id)
                .like("message", message)
                .eq("messageType", messageType)
                .eq("appId", appId)
                .eq("userId", userId);
        // 游标查询逻辑：只使用 createTime 作为游标，查询更早的消息。
        if (lastCreateTime != null) {
            queryWrapper.lt("createTime", lastCreateTime);
        }
        // 排序：如果前端指定排序字段就按前端来，否则聊天历史默认按创建时间倒序。
