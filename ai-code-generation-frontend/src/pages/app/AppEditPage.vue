      ...
      <a-card title="基本信息" :loading="loading">
        ...
        >
          ...
          <a-form-item label="可见范围" name="visibility">
            <a-select v-model:value="formData.visibility" :options="visibilityOptions" />
          </a-form-item>
          ...
          <a-form-item label="应用标签" name="tags">
            <a-select
              ...
              :max-tag-count="5"
            />
            <div class="form-tip">最多 10 个标签，单个标签不超过 20 个字符</div>
          </a-form-item>
      ...
      <a-card title="应用信息" style="margin-top: 24px">
        <a-descriptions :column="2" bordered>
          ...
          <a-descriptions-item label="可见范围">
            <a-tag :color="appInfo?.visibility === 'public' ? 'green' : 'default'">
              {{ appInfo?.visibility === 'public' ? '公开' : '私有' }}
            </a-tag>
          </a-descriptions-item>
          <a-descriptions-item label="应用标签">
            <a-space v-if="appInfo?.tags?.length" wrap>
              <a-tag v-for="tag in appInfo.tags" :key="tag">{{ tag }}</a-tag>
            </a-space>
...
const formData = reactive({
  appName: '',
  // 默认私有，只有用户明确选择公开后才进入公开/精选列表。
  visibility: 'private',
  // a-select 的 tags 模式直接维护字符串数组，提交给后端再统一格式化。
  tags: [] as string[],
  cover: '',
  priority: 0,
  initPrompt: '',
...
const visibilityOptions = [
  // 这里的 value 必须和后端 AppVisibilityEnum 保持一致。
...
const fetchAppInfo = async () => {
  ...
  try {
    ...
    if (res.data.code === 0 && res.data.data) {
      ...
      // 老数据可能没有 visibility，前端展示时按私有处理。
      formData.visibility = appInfo.value.visibility || 'private'
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isAdmin.value) {
      ...
      // 普通用户只能修改应用名称
      // 本次扩展额外允许维护可见范围和标签，但仍不能修改封面、优先级等管理员字段。
      res = await updateApp({
        id: appInfo.value.id,
        appName: formData.appName,
        visibility: formData.visibility,
