  ...
  <div id="appManagePage">
    ...
    <a-form layout="inline" :model="searchParams" @finish="doSearch">
      ...
      <a-form-item label="可见范围">
        <a-select v-model:value="searchParams.visibility" placeholder="选择可见范围" style="width: 120px">
          <a-select-option value="">全部</a-select-option>
      ...
      <a-form-item label="标签">
        <!-- 后台按单个标签筛选，方便管理员快速定位同类应用。 -->
        <a-input v-model:value="searchParams.tag" placeholder="输入标签" />
      </a-form-item>
      <a-form-item>
    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-else-if="column.dataIndex === 'visibility'">
          <a-tag :color="record.visibility === 'public' ? 'green' : 'default'">
            {{ record.visibility === 'public' ? '公开' : '私有' }}
          </a-tag>
        </template>
        <template v-else-if="column.dataIndex === 'tags'">
          <!-- 管理员列表展示完整标签，方便直接判断分类是否合理。 -->
